...
router.post('/register', async (req, res) => {
    try {
        ...
        const existingAgent = await db.get('SELECT * FROM agents WHERE address = $1', [address]);
...
router.post('/verify-auto/:token', async (req, res) => {
    try {
        ...
        const verificationResult = await db.get(
            'SELECT * FROM pending_verifications WHERE token = $1 AND status = $2',
            [token, 'pending']
        );
        ...
        await db.run(`
            UPDATE agents SET verified = 1, updated_at = NOW()
            WHERE address = $1
...
router.post('/verify/:address', async (req, res) => {
    try {
        ...
        await db.run(`
            UPDATE pending_verifications 
            SET status = 'verified', verified_at = NOW(), verified_by = 'admin'
...
router.post('/verify-risueno', async (req, res) => {
    try {
        ...
        // Verify agent
        await db.run('UPDATE agents SET verified = 1 WHERE address = $1', [address]);
        
        // Update verification status
        await db.run("UPDATE pending_verifications SET status = 'verified', verified_at = NOW(), verified_by = 'admin' WHERE agent_address = $1", [address]);
        
        // Ensure credit score exists
        const cs = await db.get('SELECT * FROM credit_scores WHERE agent_address = $1', [address]);
        if (cs.rows.length === 0) {
            await db.run('INSERT INTO credit_scores (agent_address, score, tier, max_loan_amount) VALUES ($1, 300, \'No Credit\', 25000000)', [address]);
        }
...
router.post('/cleanup', async (req, res) => {
    try {
        ...
        for (const addr of testAddresses) {
            const lr = await db.run('DELETE FROM loans WHERE borrower_address = $1 OR lender_address = $2', [addr, addr]);
            deleted.loans += lr.changes || 0;
            const cr = await db.run('DELETE FROM credit_scores WHERE agent_address = $1', [addr]);
            deleted.creditScores += cr.changes || 0;
            const vr = await db.run('DELETE FROM pending_verifications WHERE agent_address = $1', [addr]);
            deleted.verifications += vr.changes || 0;
            const ar = await db.run('DELETE FROM agents WHERE address = $1', [addr]);
        ...
        for (const a of testAgents.rows) {
            await db.run('DELETE FROM loans WHERE borrower_address = $1 OR lender_address = $2', [a.address, a.address]);
            await db.run('DELETE FROM credit_scores WHERE agent_address = $1', [a.address]);
            await db.run('DELETE FROM pending_verifications WHERE agent_address = $1', [a.address]);
            const r = await db.run('DELETE FROM agents WHERE address = $1', [a.address]);
