...
app.get('/health', async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      status: 'unhealthy', 
      timestamp: new Date().toISOString(),
      database: 'PostgreSQL (Supabase) - error',
      error: error.message
    });
  }
});
...
app.use('/api/agents', agentOnlyMiddleware, require('./routes/agents-supabase'));
app.use('/api/loans', agentOnlyMiddleware, require('./routes/loans-supabase'));
app.use('/api/messages', agentOnlyMiddleware, require('./routes/messages-supabase'));
